...
// IF NOT EXISTS guards so it can run from a serverless endpoint or a build step.
const DDL: string[] = [
...
// Foreign keys wrapped in DO blocks so re-running is a no-op (Postgres has no
// ADD CONSTRAINT IF NOT EXISTS).
const FKS: string[] = [
  `ALTER TABLE "coin_transactions" ADD CONSTRAINT "coin_transactions_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE cascade`,
  ...
  `ALTER TABLE "order_items" ADD CONSTRAINT "order_items_product_id_products_id_fk" FOREIGN KEY ("product_id") REFERENCES "public"."products"("id")`,
  `ALTER TABLE "orders" ADD CONSTRAINT "orders_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id")`,
  `ALTER TABLE "stock_movements" ADD CONSTRAINT "stock_movements_product_id_products_id_fk" FOREIGN KEY ("product_id") REFERENCES "public"."products"("id") ON DELETE cascade`,
  `ALTER TABLE "user_species" ADD CONSTRAINT "user_species_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE cascade`,
  `ALTER TABLE "user_species" ADD CONSTRAINT "user_species_species_id_species_id_fk" FOREIGN KEY ("species_id") REFERENCES "public"."species"("id") ON DELETE cascade`,
];
...
/** Create the schema (idempotent) and seed species, admin and sample products. */
export async function provisionAndSeed(db: DB): Promise<ProvisionResult> {
  ...
  for (const fk of FKS) {
    await db.execute(
      sql.raw(
        `DO $$ BEGIN ${fk}; EXCEPTION WHEN duplicate_object THEN null; WHEN duplicate_table THEN null; END $$;`
      )
    );
  }
  ...
  const adminEmail = adminEmailList()[0] ?? "terrarisztika1@gmail.com";
  ...
  if (productCount === 0) {
    await db.insert(products).values(SAMPLE_PRODUCTS).onConflictDoNothing();
    sampleProductsCreated = SAMPLE_PRODUCTS.length;
  }

  return { speciesCount, adminEmail, adminCreated, sampleProductsCreated };
}
